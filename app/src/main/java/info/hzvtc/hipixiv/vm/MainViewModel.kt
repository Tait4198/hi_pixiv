...
import info.hzvtc.hipixiv.view.fragment.EmptyFragment
import info.hzvtc.hipixiv.view.fragment.IllustFragment
import info.hzvtc.hipixiv.view.fragment.ViewPagerFragment
    ...
    override fun initViewModel() {
        ...
        newestFollowBundle = object : ViewPagerBundle<BaseFragment>() {
            init {
                ...
                pagers = arrayOf(
                        IllustFragment(obsToken.flatMap({ token -> apiService.getFollowIllusts(token,restricts[0])}),account,false)
                    ...
                    0 -> {
                        showSingleFilterDialog(R.array.newest_follow_illust_items,object : Action{
                            override fun doAction() {
                                (pagers[0] as IllustFragment).getViewModelData()?.getData(obsToken.flatMap({token ->
                                    apiService.getFollowIllusts(token,restricts[restrictPos])}))
                            }
                        })
                    }
                }
            ...
            override fun fabShow(position: Int) {
                ...
                if(position > 1) mBind.fab.hide() else mBind.fab.show()
            }
        }
    }
        ...
        if(identifier != nowIdentifier){
            ...
            when(identifier){
                ...
                MainActivity.Identifier.HOME_ILLUSTRATIONS.value -> {
                    replaceFragment(IllustFragment(obsToken.flatMap({ token ->
                ...
                MainActivity.Identifier.NEWEST_FOLLOW.value -> {
                    ...
                    replaceFragment(ViewPagerFragment(newestFollowBundle))
                    mBind.fab.show()
                }
            }
        }
